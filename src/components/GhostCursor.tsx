...
export const GhostCursor: React.FC<GhostCursorProps> = ({
  ...
  const scrollIntensityFactorRef = useRef(1.0);
  ...
  const fragmentShader = `
    ...
    vec3 tint1(vec3 base){ return mix(base, vec3(0.9), 0.1); }
    vec3 tint2(vec3 base){ return mix(base, vec3(0.6, 0.2, 0.2), 0.2); }
    ...
    vec4 blob(vec2 p, vec2 mousePos, float intensity, float activity) {
      ...
      float alpha = pow(smoke, 2.8) * distFactor;
  ...
  useEffect(() => {
    ...
    // Dynamic scroll attenuation
    let scrollTimeout: number | null = null;
    const handleScroll = () => {
      scrollIntensityFactorRef.current = 0.35; // Dim significantly while actively scrolling
      if (scrollTimeout) clearTimeout(scrollTimeout);
      scrollTimeout = window.setTimeout(() => {
        scrollIntensityFactorRef.current = 1.0; // Restore when scroll stops
      }, 150);
    };
    ...
    const animate = () => {
      ...
      // Smoothly blend scroll intensity factor
      const targetOpacity = fadeOpacityRef.current * scrollIntensityFactorRef.current;
      mat.uniforms.iOpacity.value = targetOpacity;
      mat.uniforms.iBrightness.value = brightness * scrollIntensityFactorRef.current;
