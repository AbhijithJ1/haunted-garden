...
import { Play, ShieldAlert, Sparkles, Film, ArrowRight, Eye, Flame, Lock, Radio } from 'lucide-react';
import { UNIVERSES_DATA } from '../data/universes';
import { UniverseInfo } from '../types';
import { BentoTilt } from './BentoTilt';
import { soundEngine } from '../audio/soundEngine';

interface ImmersionArchiveProps {
  onOpenTrailer: (universe: UniverseInfo) => void;
}

export const ImmersionArchive: React.FC<ImmersionArchiveProps> = ({ onOpenTrailer }) => {
  const fearDetails: Record<string, { mechanism: string; quote: string; architecture: string; evidence: string; icon: React.ReactNode }> = {
    conjuring: {
  ...
  const handlePlayCardSound = (universe: UniverseInfo) => {
    soundEngine.playCardDraw();
    onOpenTrailer(universe);
  };

  return (
    <section id="immersion-archive" className="relative w-full bg-transparent py-32 px-4 sm:px-10 text-white z-10">
      <div className="max-w-7xl mx-auto w-full">
        
        {/* Section Header with Generous Clearance */}
        <div className="mb-24 border-b border-white/[0.08] pb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <span className="font-mono text-[9px] text-red-500 uppercase tracking-widest block font-bold mb-2">
              02 // THE ACTIVE ARCHIVE PORTALS
            </span>
          ...
          <p className="font-grotesk text-xs sm:text-sm text-white/50 max-w-md font-light leading-relaxed">
            Continuous autoplaying scary scenes from each horror masterwork. 3D perspective tracking and forensic evidence dossiers.
          </p>
        </div>

        {/* The 6 Content-Packed Case Modules with 3D BentoTilt */}
        <div className="flex flex-col gap-28 sm:gap-36">
          {UNIVERSES_DATA.map((u, idx) => {
            const meta = fearDetails[u.id] || fearDetails.conjuring;

            return (
              <BentoTilt key={u.id} tiltFactor={5} className="rounded-3xl">
                <div
                  id={`case-file-0${idx + 1}`}
                  className="rounded-3xl bg-[#08080c]/90 backdrop-blur-xl border border-white/15 hover:border-red-500/60 transition-all duration-500 overflow-hidden shadow-[0_30px_90px_rgba(0,0,0,0.95)] grid grid-cols-1 lg:grid-cols-12 gap-0"
                >
                  {/* Left 7 Columns: Autoplaying 16:9 Theatrical Video Frame */}
                  <div className="lg:col-span-7 relative aspect-video min-h-[320px] sm:min-h-[420px] lg:min-h-[500px] bg-black overflow-hidden group">
                    <iframe
                      ...
                      className="absolute inset-0 w-full h-full object-cover scale-110 brightness-90 contrast-125 filter grayscale-[15%] group-hover:scale-115 transition-transform duration-1000 pointer-events-none"
                    />
                    ...
                    <div className="absolute top-5 left-5 right-5 flex items-center justify-between font-mono text-[9px] text-white/80 uppercase tracking-widest z-20 pointer-events-none">
                      <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-black/80 border border-white/20 backdrop-blur-md">
                        ...
                        <span>LIVE STREAM // CASE 0{idx + 1}</span>
                      </div>
                  ...
                  {/* Right 5 Columns: Forensic Editorial Dossier with Zero Text Clipping */}
                  <div className="lg:col-span-5 p-8 sm:p-10 flex flex-col justify-between space-y-6 bg-[#08080c]/90">
                    <div>
                      ...
                      <h3 className="font-cinzel font-black text-2xl sm:text-3xl lg:text-4xl text-white tracking-tight uppercase mt-1 leading-tight">
                        {u.title}
                      </h3>
